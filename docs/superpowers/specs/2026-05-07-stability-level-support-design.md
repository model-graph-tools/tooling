...
- The dev build (identifier 0) returns `false` since it's a special case. This is acceptable because dev builds are typically built from source with custom configuration.
- Add unit tests for boundary cases: WildFly 30 (false), 31 (true), 39 (true), dev (false).
- Bump crate version and release.
